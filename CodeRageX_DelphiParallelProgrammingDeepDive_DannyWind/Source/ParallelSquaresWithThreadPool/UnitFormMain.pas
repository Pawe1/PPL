...
{===============================================================================
 Demo for TParallel.For using ThreadPool

 The base code visually shows how Stride works when using TParallel.For.
 ...
 The ThreadPool Button uses a custom threadpool with limited resources.
...
uses
  ...
  FMX.NumberBox, System.Threading;
...
type
  TFormMain = class(TForm)
    ...
    procedure ButtonThreadPoolClick(Sender: TObject);
  private
...
procedure TFormMain.ButtonThreadPoolClick(Sender: TObject);
var
  lStride, lFrom, lTo: Integer;
begin
  if (FPool = nil) then
  begin {Create Minimal ThreadPool}
  ...
  TTask.Run(procedure
            begin
              TParallel.For(lStride, lFrom, lTo, PaintRectangle, FPool)
            end, FPool);
end;
